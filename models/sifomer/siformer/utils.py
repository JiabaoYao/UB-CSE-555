...
def train_epoch(model, dataloader, criterion, optimizer, device,scheduler=None):
    ...
    for i, data in enumerate(dataloader):
        ...
        # print(f"l_hands: {l_hands.shape}, r_hands: {r_hands.shape}, bodies: {bodies.shape}, labels: {labels.shape}")
        # print(modelgcn)
        # combined_data = torch.cat((bodies, r_hands, l_hands), dim=2)
        # print(combined_data.shape)
        
        # outgcn = modelgcn(combined_data)
        # print(outgcn.shape)
...
def evaluate(model, dataloader, device, print_stats=False):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            for j in range(labels.size(0)):
                ...
                # Statistics
                pred = int(torch.argmax(torch.nn.functional.softmax(output, dim=2)))
    ...
    return pred_correct, pred_all, (pred_correct / pred_all), all_labels, all_preds
