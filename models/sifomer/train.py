...
def get_default_args():
    ...
    parser.add_argument("--batch_size", type=int, default=16, help="Number of batch size") # default=16
    parser.add_argument("--num_worker", type=int, default=16, help="Number of workers") # default=24
...
def train(args):
    ...
    if isinstance(sample, (tuple, list)):
        print(f"Sample[0] shape: {sample[0].shape}")  # e.g., left_hand
    else:
    ...
    for epoch in range(args.epochs):
        ...
        if val_loader:
            ...
            _, _, val_acc, labels, preds = evaluate(slr_model, val_loader, device)
            slr_model.train(True)
    ...
    if eval_loader:
        for i in range(checkpoint_index):
            for checkpoint_id in ["t", "v"]:
                ...
                _, _, eval_acc, labels, preds = evaluate(tested_model, eval_loader, device, print_stats=True)
    ...
    df_acc = pd.DataFrame({
        "Epoch": np.arange(len(test_acc)),
        "Train Accuracy": train_accs,
        "Test Accuracy": test_acc
    })
    df_acc.to_csv("siformer-50words-67joints-accuracy.csv", index=False)

    df_preds = pd.DataFrame({
        "True Label": all_labels,
        "Predicted Label": all_preds
    })
    df_preds.to_csv("CNN-LSTM-50words-67joints-predictions.csv", index=False)
