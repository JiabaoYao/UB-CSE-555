...
class GlossKeypointDataset(Dataset):
    def __init__(self):
        ...
        self.video_matrix = np.load(os.path.join(base_path, "dataset", "dataset_padding_new.npz"), allow_pickle=True)['data']  # shape: [N, 2]
        self.n_samples = len(self.video_matrix)

        # build gloss index map
        glosses = [row[0] for row in self.video_matrix]
        self.gloss_to_index = {gloss: idx for idx, gloss in enumerate(sorted(set(glosses)))}
        self.index_to_gloss = {idx: gloss for gloss, idx in self.gloss_to_index.items()}
    ...
    def __getitem__(self, idx): # return single video information
        ...
        gloss_str = self.video_matrix[idx][0]
        gloss = torch.tensor(self.gloss_to_index[gloss_str], dtype=torch.long)
